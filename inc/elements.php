...
function image($ID, $size, $class = '', $alt = 'Feature Image', $loading = 'lazy') {
    // Prevent output during admin operations
    if (is_admin() && (wp_doing_ajax() || (isset($_POST) && !empty($_POST)))) {
        return;
    }
    
    // Handle auto width with fixed height - with safe array access
...
function button($text, $link = '#', $target = '_self') {
    // Prevent ALL execution during admin operations
    if (is_admin()) {
        return;
    }
    
    // Additional safety check for AJAX and POST operations
    ...
    // Safe icon handling with additional checks
    if ($is_external_link && function_exists('icon_external_link')) {
        // Use output buffering safely
        ob_start();
        try {
            icon_external_link();
    ...
    // Safe output with validation
    if (!empty($text)) {
        echo '<a href="' . esc_url($link) . '" class="button" target="' . esc_attr($target) . '">' . esc_html($text) . $icon_svg . '</a>';
    }
} 